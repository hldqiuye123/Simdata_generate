...
def load_dataloader(num_samples, signal_dim, max_n_freq, min_sep, distance, amplitude, floor_amplitude,
    ...
    clean_signals, f, nfreq ,r,s_Fourier,theta_set= data.gen_signal(num_samples, signal_dim, max_n_freq, min_sep, distance=distance,
                                              amplitude=amplitude, floor_amplitude=floor_amplitude,
                                              variable_num_freq=True)
    frequency_representation_amp,frequency_representation_phase,fr_ground,m1,m2 = fr.freq2fr(f, xgrid, kernel_type, kernel_param,r,nfreq,theta_set)


    clean_signals = torch.from_numpy(clean_signals).float()
    f = torch.from_numpy(f).float()
    frequency_representation_amp = torch.from_numpy(frequency_representation_amp)
    frequency_representation_phase = torch.from_numpy(frequency_representation_phase)
    fr_ground = torch.from_numpy(fr_ground).float()
    m1 = torch.from_numpy(m1).float()
    m2 = torch.from_numpy(m2).float()
    dataset = data_utils.TensorDataset(clean_signals, frequency_representation_amp,frequency_representation_phase, f,fr_ground,m1,m2,torch.tensor(theta_set))
    return data_utils.DataLoader(dataset, batch_size=batch_size, shuffle=False)
...
def load_dataloader_fixed_noise(num_samples, signal_dim, max_n_freq, min_sep, distance, amplitude, floor_amplitude,
    ...
    clean_signals, f, nfreq,r,s_Fourier,theta_set = data.gen_signal(num_samples, signal_dim, max_n_freq, min_sep, distance=distance,
                                              amplitude=amplitude, floor_amplitude=floor_amplitude,
                                              variable_num_freq=True)
    frequency_representation,frequency_representation_phase,fr_ground,m1,m2 = fr.freq2fr(f, xgrid, kernel_type, kernel_param,r,nfreq,theta_set)

    clean_signals = torch.from_numpy(clean_signals).float()
    f = torch.from_numpy(f).float()
    frequency_representation = torch.from_numpy(frequency_representation)
    frequency_representation_phase = torch.from_numpy(frequency_representation_phase)
    fr_ground = torch.from_numpy(fr_ground).float()
    ...
    m1 = torch.from_numpy(m1).float()
    m2 = torch.from_numpy(m2).float()
    dataset = data_utils.TensorDataset(noisy_signals, clean_signals, frequency_representation,frequency_representation_phase, f,fr_ground,m1,m2)
